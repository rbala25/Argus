...
Follow-on to [Hermes](https://github.com/rbala25/Hermes), another project I did involving a market-making engine with a full Ethernet/TCP/IP stack. Where Hermes was more so about getting packets through a pipeline with minimal latency, this project was mainly an excuse to learn UVM properly and do memory hierarchy work along the way. I verified Hermes with plain directed testbenches, and I wanted something small enough to build a real UVM environment around from scratch.

## Verification

The whole design runs through one UVM environment: driver, monitor, sequencer, scoreboard, agent, env. The scoreboard checks matches against a Python-generated reference model and logs cache hit/miss counts at end of test, so every eviction policy gets judged by the same testbench instead of ad hoc checks. Building this out (virtual interfaces, config db, the usual UVM plumbing) was honestly most of the work.
...
- UVM testbench as described above
